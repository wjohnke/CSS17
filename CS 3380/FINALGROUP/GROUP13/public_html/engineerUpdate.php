...
<?php
	if(isset($_POST['update'])){
		...
		$query = $query = "SELECT `EMPLOYEE`.Employee_Id AS 'Employee ID', `EMPLOYEE`.Train_Assigned AS 'Train Assigned', `ENGINEER`.Hours AS 'Hours', `ENGINEER`.Status AS 'Status' FROM `EMPLOYEE` JOIN `ENGINEER` ON `EMPLOYEE`.Employee_Id = `ENGINEER`.Employee_Id WHERE `EMPLOYEE`.Employee_Id ='$pk';";
...
<nav class="w3-sidebar w3-red w3-collapse w3-top w3-large w3-padding" style="z-index:3;width:300px;font-weight:bold;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
	...
	<div align="center">
		<form action="handleEngineerUpdate.php" method="POST">
			ID Number:<br>
			(Unable to edit)<br>
			...
			<br><br>
			Hours:<br>
			<input type='text' style="text-align: center" name='hours' value= '<?=$row[2]?>'></input>
			<br><br>
			Train Slot:<br>
			<input type="radio" name="radioChoice" value="1" checked="check">Engineer #1</input>
			<br>
			<input type="radio" name="radioChoice" value="2">Engineer #2</input>
			<br><br>
			Status:<br>
