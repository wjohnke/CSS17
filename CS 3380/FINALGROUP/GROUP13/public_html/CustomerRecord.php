...
if($accessLevel=="admin"){
	...
	<form action="" method="POST">
		Order Number: <input type="text" name="order_num" placeholder="Search by Order Number">
		<br><br>
		Sort By Most Recent: <input type="radio" name="dateASC" value="DESC"><br>
		Sort By Oldest: <input type="radio" name="dateASC" value="ASC"><br>
		Number of results to display: <input type="number" name="maxNum"><br>
		<input type="submit" name="submit" value="Search">
	</form>
	<?php
	
	if(isset($_POST['submit']) && $_POST['submit']=="Search"){
		if(isset($_POST['dateASC']) && $_POST['dateASC']=="ASC") $sortByDateDESC=false;
		if(!empty($_POST['order_num']) ){
			$order_number=htmlspecialchars($_POST['order_num'] );
			$sql .=" WHERE Order_Num = ? ";
			$sql.=" ORDER BY Date_Accessed ";
			if($sortByDateDESC) $sql .= "DESC";
			else $sql .= "ASC";
			if(isset($_POST['maxNum']) && $_POST['maxNum']>0) $sql .= " LIMIT " . $_POST['maxNum'];
			$sql .=";";
		...
		if($exists==0) echo "No results were found, please try again";
		else{
			echo "<table>";
	...
	$sql="SELECT * FROM CUSTOMER_LOG WHERE Customer_Id=" . $_SESSION['id'];
	?>
	<form action="" method="POST">
		Order Number: <input type="text" name="order_num" placeholder="Search by Order Number">
		<br><br>
