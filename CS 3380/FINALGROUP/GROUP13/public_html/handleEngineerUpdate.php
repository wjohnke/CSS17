...
<?php
	if(isset($_POST['submit'])){
		...
		$query = "UPDATE `ENGINEER` SET `Hours`=?, `Status`=? WHERE `Employee_Id`=?;";
		$stmt = $link->stmt_init();
		if(!$stmt->prepare($query)){
			echo "Could not load second query";
			exit();
		}
		$stmt->bind_param("sss", $_POST['hours'], $_POST['status'], $_POST['id']);
		if($stmt->execute()){
		...
		if($_POST['radioChoice']=='1'){
			$query = "UPDATE `TRAIN` SET `Engineer_Assigned_1`=? WHERE `Train_Num`=?;";
			$stmt = $link->stmt_init();
			if(!$stmt->prepare($query)){
		...
		if($_POST['radioChoice']=='2'){
			$query = "UPDATE `TRAIN` SET `Engineer_Assigned_2`=? WHERE `Train_Num`=?;";
			$stmt = $link->stmt_init();
			if(!$stmt->prepare($query)){
				echo "Could not load third query";
				exit();
