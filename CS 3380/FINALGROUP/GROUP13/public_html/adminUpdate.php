...
<?php
	if(isset($_POST['update'])){
		...
		$query = $query = "SELECT `EMPLOYEE`.Employee_Id AS 'Employee ID', `EMPLOYEE`.Train_Assigned AS 'Train Assigned', `ADMIN`.Location AS 'Location' FROM `EMPLOYEE` JOIN `ADMIN` ON `EMPLOYEE`.Employee_Id = `ADMIN`.Employee_Id WHERE `EMPLOYEE`.Employee_Id ='$pk';";
...
<nav class="w3-sidebar w3-red w3-collapse w3-top w3-large w3-padding" style="z-index:3;width:300px;font-weight:bold;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
	...
	<div align="center">
		<form action="handleAdminUpdate.php" method="POST">
			ID Number:<br>
			(Unable to edit)<br>
			...
			<br>
			Location:<br>
			<input type='text' style="text-align: center" name='location' value= '<?=$row[2]?>'></input>
			<br>
