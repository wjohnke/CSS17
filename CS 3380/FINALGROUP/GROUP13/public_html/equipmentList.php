...
function executeQuery($mysqli, $query){
	...
	echo "<br>" . $exists . " cars were found<br><br>";
	echo "<table>";
	...
	while($row=$result->fetch_array(MYSQLI_NUM)) {
		...
		<td>
			...
			<form action='reserve.php' method="POST">
				...
				<input type="hidden" name="Local" value='<?= $row[0] ?>'>
...
*/
	$carLocation= (empty($_GET['carLocation']) ? "" :$_GET['carLocation'] );
	$carType= (empty($_GET['carType']) ? "" : $_GET['carType']) ;
	...
	*/
	...
	$query="SELECT Car_Id_NUM AS Car_ID, `Location`,Manufacturer, `Car_Type` AS CarType, `Load_Capacity` AS LoadCapacity, `Price` FROM `EQUIPMENT` WHERE Reserve_Status='open' ";
